class_name MovementAbility extends RPGAbility
...
func can_equip_ability():
	for ability in warrior.ability_list:
		if ability is MovementAbility and ability.equipped:
			OnScreenMessageMaster.display_message("Cannot Equip More than One Movement Ability")
			return false
	return super()
